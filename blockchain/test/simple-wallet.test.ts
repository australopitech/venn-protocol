...
describe.skip('RWallet', function () {
  ...
  describe('#validateUserOp', () => {
    ...
    before(async () => {
      ...
      // cant use "RWalletFactory", since it attempts to increment nonce first
      const implementation = await new RWallet__factory(ethersSigner).deploy(entryPointEoa)
      const proxy = await new ERC1967Proxy__factory(ethersSigner).deploy(implementation.address, '0x')
      account = RWallet__factory.connect(proxy.address, epAsSigner)
  ...
  context('RWalletFactory', () => {
    it('sanity: check deployer', async () => {
      const ownerAddr = createAddress()
      const deployer = await new RWalletFactory__factory(ethersSigner).deploy(entryPoint)
